...
public func createCorrectTaskManager() -> PTNet {
    ...
    let processPool = PTPlace(named: "processPool")
    let inProgress  = PTPlace(named: "inProgress")
    let FreeProcess = PTPlace(named : "FreeProcess")
    ...
    let success     = PTTransition(
        named          : "success",
        preconditions  : [PTArc(place: taskPool), PTArc(place: inProgress), PTArc(place: FreeProcess)],
        postconditions : [PTArc(place: FreeProcess)])
    let exec       = PTTransition(
        named          : "exec",
        preconditions  : [PTArc(place: taskPool), PTArc(place: processPool), PTArc(place: FreeProcess)],
        postconditions : [PTArc(place: taskPool), PTArc(place: inProgress)])
    let fail        = PTTransition(
        named          : "fail",
        preconditions  : [PTArc(place: inProgress)],
    ...
    // P/T-net
    return PTNet(
        places: [taskPool, processPool, inProgress, FreeProcess],
        transitions: [create, spawn, success, exec, fail])
}
